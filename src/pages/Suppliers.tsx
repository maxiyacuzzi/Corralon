  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-300 mb-1">CUIT</label>
          <input
            value={taxId}
  ...
  return (
    <div className="space-y-6">
      ...
      {showForm && (
        <div className="rounded-xl border border-gray-700 bg-gray-800 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-medium text-white">Nuevo proveedor</h2>
      ...
      <div className="rounded-xl border border-gray-700 bg-gray-800 overflow-x-auto">
        {loading ? (
          <p className="p-5 text-gray-400">Cargando proveedores...</p>
        ) : suppliers.length === 0 ? (
          <p className="p-5 text-gray-400">No hay proveedores cargados todavía.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody>
              {suppliers.map((supplier) => (
                <tr key={supplier.id} className="border-b border-gray-800 last:border-0">
                  <td className="px-5 py-3 text-white font-medium">{supplier.name}</td>
                  <td className="px-5 py-3 text-gray-300">{supplier.tax_id ?? '—'}</td>
                  <td className="px-5 py-3 text-gray-300">{supplier.phone ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
