    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        {availableNotes.length === 0 ? (
          <p className="text-sm text-gray-500">Este cliente no tiene remitos pendientes de facturar.</p>
        ) : (
          <div className="max-h-40 overflow-y-auto rounded-lg border border-gray-700 divide-y divide-gray-800">
            {availableNotes.map((note) => (
              <label
                key={note.id}
                className="flex items-center justify-between gap-3 px-3 py-2 text-sm text-gray-300 cursor-pointer hover:bg-gray-800/60"
              >
                ...
                <span className="text-white font-medium">${deliveryNoteTotal(note, productsById).toFixed(2)}</span>
              </label>
            ))}
          </div>
        )}
        {selectedNoteIds.length > 0 && (
          <p className="mt-2 text-sm text-gray-400">
            Total de remitos seleccionados (a precio actual): <span className="text-white font-medium">${selectedNotesTotal.toFixed(2)}</span> — repartilo entre las formas de pago de abajo.
          </p>
        )}
      </div>

      <div className="space-y-3">
        <label className="block text-sm font-medium text-gray-300">Formas de pago</label>
        {paymentLines.map((line, index) => (
          <div key={index} className="space-y-3 rounded-lg border border-gray-700 p-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              {line.method === 'cash' && (
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">Descuento efectivo (%)</label>
                  <input
                    type="number"
                    min={0}
                    max={100}
            ...
            {line.method === 'cash' && (Number(line.discountPercent) || 0) > 0 && (
              <p className="text-sm text-gray-400">
                Monto final de esta línea: <span className="text-white font-medium">${paymentLineFinalAmount(line).toFixed(2)}</span>
              </p>
            )}

            {line.method === 'checks' && (
              <div className="space-y-3 border-t border-gray-800 pt-3">
                <p className="text-xs font-medium text-gray-400">Datos del cheque</p>
                <div className="grid grid-cols-2 gap-4">
                  <input
                    required
      ...
      <p className="text-right text-white font-medium">Total: ${totalFinal.toFixed(2)}</p>
    ...
    <div className="space-y-6">
      ...
      {previewSale && (
        <div className="space-y-3">
          <div className="print:hidden flex items-center justify-between">
            <button onClick={() => setPreviewSale(null)} className="text-sm text-gray-400 hover:text-white">
              ← Volver al listado
            </button>
      ...
      <div className="rounded-xl border border-gray-700 bg-gray-800 overflow-x-auto">
        {loading ? (
          ...
          <p className="p-5 text-gray-400">Ninguna venta coincide con la búsqueda.</p>
        ) : (
